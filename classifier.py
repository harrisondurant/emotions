...
def run_svm(params, X):
    ...
    for trial in range(num_trials):
        ...
        for train_idx, test_idx in kf:
            ...
            print('Trial %d, Fold %d of %d: Accuracy = %.2f' % (trial+1, count, num_folds, acc))
            acc_sum += acc
            count += 1

        print('Mean Accuracy after %d folds = %.2f' % (num_folds, acc_sum / num_folds))
        print()
        overall_acc += acc_sum / num_folds

    print('Mean Accuracy after %d trials = %.2f' % (num_trials, overall_acc / num_trials))

    cnf /= (num_folds * num_trials)
